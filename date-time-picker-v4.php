...
if (!class_exists('ACFFieldDateTimePicker')) :
    ...
    class ACFFieldDateTimePicker extends acf_field {
        ...
        function __construct($opts = array()) {
            ...
            // settings
            $this->settings = array(
                'path' => isset($opts['url']) ? $opts['url'] : plugin_dir_url(__FILE__),
                'dir' => isset($opts['path']) ? $opts['path'] : plugin_dir_path(__FILE__),
                'timezone' => isset($opts['timezone']) ? new \DateTimeZone($opts['timezone']) : new \DateTimeZone(get_option('timezone_string')),
        ...
        function format_value($value, $post_id, $field) {
            ...
            $time->setTimezone($this->settings['timezone']);
            ...
            if ('' != $value && 'true' == $field['save_as_timestamp'] && $this->isValidTimeStamp($value)) {
                if ('true' == $field['show_date']) {
                    ...
                    $value = $field['save_as_timestamp'] && !empty($value) ? $time->format($format) : '';
                } else {
        ...
        function format_value_for_api($value, $post_id, $field) {
            $time = new \DateTime($this->settings['timezone']);
            $time->setTimestamp($field['value']);
            $time->setTimezone($this->settings['timezone']);
        ...
        function input_admin_enqueue_scripts() {
            ...
            wp_enqueue_script('jquery-ui-timepicker', $this->settings['path'] . 'js/jquery-ui-timepicker-addon.js', array(
                'acf-input',
                'jquery-ui-slider',
                    ), $this->settings['version'], true);

            if (file_exists($this->settings['dir'] . '/js/localization/jquery-ui-timepicker-' . $js_locale . '.js')) {
                wp_enqueue_script('timepicker-localization', $this->settings['path'] . 'js/localization/jquery-ui-timepicker-' . $js_locale . '.js', array(
                    'jquery-ui-timepicker'
                        ), $this->settings['version'], true);
                wp_enqueue_script('timepicker', $this->settings['path'] . 'js/timepicker.js', array(
            ...
            wp_enqueue_style('jquery-style', $this->settings['path'] . 'css/jquery-ui.css', array(
                'acf-datepicker'
                    ), $this->settings['version']);
            wp_enqueue_style('timepicker', $this->settings['path'] . 'css/jquery-ui-timepicker-addon.css', array(
                'jquery-style'
                    ), $this->settings['version']);
        }
        ...
        function get_js_locale($locale) {
            ...
            $dir_path = $this->settings['dir'] . 'js/localization/';
            $exclude_list = array('.', '..');
